...
import com.github.cxfplus.core.reflect.BeanUtils.SearchMode;
...
public class ClassEx {
	private static Logger log=LoggerFactory.getLogger(ClassEx.class);
	/**
	 * Class对象
	 */
	private Class<?> cls;
	...
	public Type getMethodParamType(Method method, int index) {
		...
		if (index < 0 || index > types.length) {
			String message=String.format("the method %1$s has %2$d params, index= %3$d is out of bound.",method.getName(),types.length,index);
	...
	private Method getRealMethod(Method method) {
		...
		if (isCGLibProxy(cls)) {
			try {
				...
				log.error("getRealMethod error.",e);
	...
	public MethodEx getMethodIfExist(String name, Class<?>... paramTypes) {
		try {
			...
			log.error("getMethodIfExist error.",e);
		} catch (NoSuchMethodException e) {
			log.error("getMethodIfExist error.",e);
		}
		return null;
	...
	public boolean isCollection() {
		return isCollection(genericType);
	}
	...
	public Type getComponentType() {
		return getComponentType(genericType);
	}
