 ...
 * <ol>
 * <li>
 * 静态代码生成Stub类型           （流行，但不推荐）  类型:WSDL优先，但在具体执行时依照本地代码中的注解形成客户端模型，实质上是代码优先。
 ...
 *    </li><li>
 * 使用接口类 生成Client。                          类型：根据本地接口类的注解形成客户端模型，代码优先
 *    调用时类似反射的方式。但由于接口中引用的POJO都在本地环境中的关系，调用相对容易。
 *    </li><li> 
 * 使用接口类 生成动态代理 （流行，推荐）  类型：根据本地接口类的注解形成客户端模型，代码优先
 ...
 *    </li><li>
 * 纯动态客户端，依赖（WSDL）生成客户端，（不流行，一定条件下推荐）  类型：根据WSDL形成客户端模型，WSDL优先
