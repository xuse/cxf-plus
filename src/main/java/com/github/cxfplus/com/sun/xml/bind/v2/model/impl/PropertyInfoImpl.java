...
import com.github.cxfplus.com.sun.xml.bind.v2.model.annotation.Locatable;
import com.github.cxfplus.com.sun.xml.bind.v2.model.core.Adapter;
import com.github.cxfplus.com.sun.xml.bind.v2.model.core.ID;
...
abstract class PropertyInfoImpl<T, C, F, M> implements PropertyInfo<T, C>, Locatable, Comparable<PropertyInfoImpl> {
	...
	public T getIndividualType() {
		T raw = getRawType();
		if (adapter != null)
			return applyGenerics(adapter.defaultType, raw);
		if (!isCollection()) {
			return raw;
		} else {
	...
	private T applyGenerics(T toType, T raw) {
		if (this.isCollection) {
			raw = unwrapCollection(raw);// 必须脱壳
		}
		if (raw instanceof ParameterizedType && toType instanceof ParameterizedType) {
			ParameterizedType from = (ParameterizedType) raw;
			ParameterizedType to = (ParameterizedType) toType;
			Type[] argF = from.getActualTypeArguments();
			Type[] argT = to.getActualTypeArguments();
			int appCount = 0;
			for (int n = 0; n < argT.length; n++) {
				Type t = argT[n];
				if (t instanceof TypeVariable<?>) {
					argT[n] = argF[appCount];
					appCount++;
				}
			}
			return (T) GenericUtils.newGenericType((Class<?>) to.getRawType(), argT);
		}
		return toType;
	}

	@SuppressWarnings("unchecked")
	private T unwrapCollection(T raw) {
		if (raw instanceof Class) {
			return (T) ((Class) raw).getComponentType();
		}
		if (raw instanceof GenericArrayType) {
			return (T) ((GenericArrayType) raw).getGenericComponentType();
		} else if (raw instanceof ParameterizedType) { // 必然是一个泛型集合
			Type t = ((ParameterizedType) raw).getActualTypeArguments()[0];
			return (T) t;
		} else {
			throw new IllegalArgumentException("Unknown collection type!" + raw);
		}
	}
	...
	/**
	 * This is the default implementation of the getAdapter method defined on
	 * many of the {@link PropertyInfo}-derived classes.
	 */
	...
	public String toString() {
		...
		sb.append(parent.getName() + "." + seed.getName());
		return sb.toString();
	}
	...
	/**
	 * Called after all the {@link TypeInfo}s are collected into the governing
	 * {@link TypeInfoSet}.
	 * 
		...
		if (id == ID.IDREF) {
			...
			for (TypeInfo<T, C> ti : ref()) {
				if (!ti.canBeReferencedByIDREF())
					parent.builder.reportError(new IllegalAnnotationException(Messages.INVALID_IDREF.format(parent.builder.nav.getTypeName(ti.getType())), this));
			}
		}
	}
	...
	//
	/**
	 * Computes the tag name from a {@link XmlElement} by taking the defaulting
	 * into account.
	 */
	...
	/**
	 * Computes the tag name from a {@link XmlElementWrapper} by taking the
	...
	private String getQualifiedNs(XmlSchema xs) {
		String uri;
